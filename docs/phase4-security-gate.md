...
```bash
python3 scripts/redteam_battery.py   # self-bootstraps /tmp/sp_redteam/
```
...
|-----|------------|---------|
| `statepod_9360` | glm-5.3-flash via failover proxy | `MaxTurnsExceeded` — empty tool calls; 0 findings |
| `statepod_6055` | mistral-small-latest via proxy | same; 0 findings; ~$0.019 |
| `statepod_bdd4` | mistral + `max_tokens` clamp | same pattern (stopped) |

Blockers (not StatePod code defects):
...
(2) statepod-node HTTP UI/API (/api/state, /api/config, /api/gov/*, /qr.js),
...
Or: `statepod collaborate --scan-mode quick` once Strix env works headlessly.
...
See `docs/adv-fix-graph.md`. Landed: localhost-only `/api/gov/issue`, authenticated `join_ok`/`member_added`, default-deny empty allow (open via `--demo`/`--open-mesh`), `gov_ui` no default secret + `127.0.0.1`, LoRa `require_psk` / `SP_LORA_REQUIRE_PSK`, find `-fprintf` denylist. Strix gate still open.
