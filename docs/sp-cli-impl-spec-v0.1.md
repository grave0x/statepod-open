# sp CLI Implementation Spec v0.1

**Status:** IMPLEMENTED (v1 + v2 shipped; 2026-08-29)
**Companion to:** `docs/sp-cli-ux-spec-v0.1.md` (the UX surface this implements)
**Entry point:** `bin/statepod` (canonical), installed as `~/.local/bin/statepod`; `sp` is a one-release compat shim

---

## 1. Architecture

`sp` is a thin CLI over the existing Python harness. It does NOT duplicate
the orchestrator: it reuses `Orchestrator.ask` (plan → gate → execute →
retry ladder), the shared C kernel (`py.statepod.StatePod`), the mesh
daemon (`harness/meshd.py`), and the governance layer
(`harness/governance.py`).

```
bin/sp ──> harness/orchestrator.py  (ask / plan_callback seam)
        ──> harness/governance.py   (gate, tokens, audit)
        ──> harness/meshd.py        (--mesh inference broker)
        ──> py/statepod.py        (C kernel: READ/WRITE/EXECUTE/registry)
        ──> ~/.statepod/          (session, rates, audit, work root)
```

## 2. Persistent state (`~/.statepod`)

| File | Purpose |
|---|---|
| `session.json` | last query/plan/result, checkpoint sha, pool context, mesh daemon info |
| `rates.json` | sp's cross-process registry memory: `{sig: {ok, total}}` from `sp feedback` |
| `audit.jsonl` | governance audit ledger (written when `--governance` is on) |
| `work/` | git-seeded repo the kernel operates on (`build_repo` corpus); `.statepod/` is gitignored |
| `work/.statepod/` | kernel state (events, checkpoints) — never committed |

**Why a rates sidecar?** The C kernel's registry is **in-memory per process**
(`sp_state_new` never loads perf from disk; `sp_state_load` rewrites repo
...
| Command | Path | Notes |
|---|---|---|
| `sp plan "q"` | `_plan_only` (make_plan + validate + blind-write guard) | never executes |
| `sp ask "q"` | `Orchestrator.ask(plan_callback=confirm)` | ONE planning pass (see §5) |
| `sp run` | stored plan → governance gate → `_exec` | re-executes the approved plan |
| `sp diff` | `git diff HEAD` in the work root | |
| `sp rollback` | `git checkout <checkpoint> -- .` | checkpoint = pre-ask HEAD |
| `sp explain <path>` | kernel `read` + `ast_parse` | |
| `sp stats` | kernel status + rates sidecar | |
| `sp feedback ok/weak` | `state.feedback(sig, ok)` + sidecar | per-op signatures of the last plan |
| `sp mesh` | status (no daemon managed by sp) | |
| `sp pool` | session pool context | set with `--pool <name>` |
| `sp gov audit/issue/verify` | audit ledger + `issue_token`/`verify_token` | see §4 |
| `sp research <query\|urls>` | `research.gather` + `research.write_kit` | keyless search (bing/ddg/wikipedia/arxiv), fetch + decode (html/pdf/office/rtf/json/xml/csv/txt) → deep-analysis-labelled kit under `./research/<topic>/`; narrative via `deep_synthesize` when a brain is configured (see README, `harness/research.py`) |
| `sp demo` | `scripts/pitch_demo.sh` | guided pitch mode |
| `sp eval sync\|list\|run [pack\|names]` | `cmd_eval` → `frontierharness.sync/select/run` | FrontierHarness Eval through sp's one-shot path (`harness/main.py`) in isolated seeded scratch repos; outcomes `DENY`/`FAIL`/`TIMEOUT`/`ERROR` — `PASS` unreachable (FHE verifiers private); report → `eval/<ts>/report.md` + `results.jsonl`; records `last_eval`; `--backend mock` runs offline |
...
- `interactive` reads stdin for BOTH the plan confirmation and the human
  approval (one terminal session: `sp ask ... --governance interactive`).
...
- `SW_BACKEND=mock` forces the deterministic mock planner (offline tests,
  no ollama). `STATEPOD_LIB` points at `libstatepod.so`.
...
| `sp demo` (guided pitch mode) | SHIPPED (wraps `scripts/pitch_demo.sh`, end-to-end validated) |
...
| Role awareness (user/support/architect) | **DORMANT (defined, not enforced)** — identity store shipped (`sp identity issue/import/show/clear`); roles + CAPS table in `harness/identity.py`; base CLI runs unrestricted (coding harness); enforcement to be switched on later or wrapped (MSP/enterprise). Expiry reported, would-degrade to user; no identity = unrestricted |
| `sp collaborate` (Strix pentest + findings fold-back) | SHIPPED — headless `strix -n` scan of a local repo/URL; summary -> `~/.statepod/strix/<run>.md`, `session.json.last_strix`, remediation hint feeds `sp ask`; `--findings [RUN]` (no new scan), `--detach`, `--console` (`strix view --no-open`); module `harness/strix_collab.py` |
| 5-minute demo without env export / manual server start | SHIPPED (PATH install; `sp demo`) |
