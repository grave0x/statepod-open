# SwarmState Build Plan v1 — LOCKED

**Date:** 2026-09-04 · **Provenance:** wayfinder map
[Feature completion + auto-tuning spine](https://github.com/grave0x/swarmstate/issues/1)
...
|---|---|---|---|
| 0 | Corpus maturity bench | **Done 2026-09-04:** `scripts/maturity_bench.py` (`make maturity-bench`) — mock A/B heuristic vs rate-based over 42 tasks; report `~/.local/state/swarmstate/maturity-bench-latest.json`; latest run pass (1.0/1.0, override honored). Live-LLM confirmation deferred (ponytail: mock proves wiring). | 10 |
...
| 2a | mingw spike (half-day timebox) | **FAIL 2026-09-04** — no mingw `libllama`/headers in distro; Linux `llama.h` conflicts with mingw `uintptr_t`. Re-scoped: keep Ollama on Windows one release; Linux `SS_EMBED_INFER` unchanged. MSVC/static llama.cpp is a future effort. | 11 |
...
| 3 | Watch loop D1 → D2 | **Done 2026-09-04:** `win/swarmstate-node.c` `watch_loop` — D1 disk/mem/load1 + registry/hash + mesh peers → `alert_add`; D2 restart link / rejoin pool / resend ops (`hist_flush_all`). Service restarts stay escalation-only. | Support/Ops |
...
| 6 | LoRa hardening | **Done 2026-09-04:** `harness/lora.py` — AES-256-GCM (PSK/`SS_LORA_PSK`), priority TX (ACK>high>bulk), per-stream route reliability cache. Hardware half of GAPS 15 still open. | 15 |
...
- `ss_export_lora` / any C-side LoRA trainer (fine-tuning, if ever, exports
