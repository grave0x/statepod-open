 ...
 * ss_infer_generate() directly (no HTTP hop, no separate daemon).
 *
 ...
 */
#ifndef SS_LLAMA_INFER_H
#define SS_LLAMA_INFER_H
 ...
 * system prompt) into the KV cache once; every ss_infer_generate() with
...
int ss_infer_is_available(void);

/* The embedded plan GBNF (llama.cpp-safe; same schema as
 * harness/plan_grammar.py PLAN_GBNF). */
const char* ss_infer_plan_gbnf(void);

#endif /* SS_LLAMA_INFER_H */
