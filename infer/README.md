...
- `llama_infer.h` / `llama_infer.c` — the `sp_infer_*` API (init, free,
...
## Run
    ...
    SP_INFER_N_THREADS=2 nice -n 19 ./infer/smoke_infer \
        ~/.ollama/models/blobs/<1.5b-gguf-sha> infer/plan.gbnf 256 48

Model directory (spec v1 §5.1): `~/.local/share/statepod/models`.

## Status

- M1 (CPU-only): implemented + node round-trip VERIFIED 2026-09-02.
  `win/statepod-node.c --embed` runs embedded-first with Ollama
...
- M2 (prefix KV cache): implemented + verified 2026-09-02.
  `sp_infer_cache_prefix(ctx, sys)` decodes the static system prompt
  once; `sp_infer_generate()` with a matching system_prompt continues
  the KV sequence at the prefix boundary (only the user prompt +
  generation tail are decoded).  The node caches `SP_PLAN_SYS` at model
  load (~105 tokens); round-trip latency dropped ~2x (13.9s -> 6.7s on
  the same query).  Fallback to the full-prompt path is automatic.
- M3 (GPU auto-tune): `sp_infer_auto_tune` heuristic shipped; runtime
