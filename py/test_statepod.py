"""StatePod Python integration tests (Phase 1).

Run:  python3 py/test_statepod.py
Requires: libstatepod.so built (`make`), git on PATH.
"""
import hashlib
import os
import subprocess
...
class StatePodTests(unittest.TestCase):
    def setUp(self):
        self._tmp = tempfile.TemporaryDirectory(prefix="statepod-py-")
    ...
    def test_legacy_snap4_loads(self):
        ...
        with open(snap, "w") as f:
            f.write("STATEPOD-SNAP-4\nroot=" + self.root + "\nserial=0\nckpt=0\nfiles=1\n")
            f.write("path=a.txt\nlen=5\nhello\n")
            f.write("perf=2\n")
            f.write("sig=READ\nkernel=c\ntotal=100\ncount=1\nok=1\nfb=1/1\n")
            f.write("sig=WRITE\nkernel=c\ntotal=50\ncount=1\nok=1\nfb=1/1\n")
        with StatePod.load(snap) as s:
    ...
    def test_write_read_grep_roundtrip(self):
        with StatePod(self.root) as s:
            ...
            self.assertEqual(
                s.read("greptest.txt", strategy=SP_CONTEXT_FULL),
                "alpha line\nbeta line\nALPHA again\nnothing here\n",
            )
    ...
    def test_line_range(self):
        with StatePod(self.root) as s:
            s.write("f.txt", "l1\nl2\nl3\nl4\n")
            self.assertEqual(s.read("f.txt", 2, 3, SP_CONTEXT_FULL), "l2\nl3\n")
            self.assertEqual(s.read("f.txt", 4, 99, SP_CONTEXT_FULL), "l4\n")
    ...
    def test_delta_containment(self):
        ...
        with StatePod(self.root) as s:
            ...
            full = s.read("big.txt", strategy=SP_CONTEXT_FULL)
            delta = s.read("big.txt", strategy=SP_CONTEXT_DELTA)
    ...
    def test_batch_and_touched_files(self):
        with StatePod(self.root) as s:
            r = s.execute([
                {"type": SP_OP_WRITE, "path": "a.txt", "content": "x"},
                {"type": SP_OP_WRITE, "path": "sub/b.txt", "content": "y"},
                {"type": SP_OP_GREP, "pattern": "y", "target": ""},
            ])
    ...
    def test_path_safety(self):
        with StatePod(self.root) as s:
            s.write("ok.txt", "safe")
            r = s.execute([{"type": SP_OP_READ, "path": "../escape"}])
            self.assertFalse(r.ok)
            self.assertIn("unsafe path", r.logs[0])
            r = s.execute([{"type": SP_OP_READ, "path": "/etc/passwd"}])
            self.assertFalse(r.ok)
            self.assertIn("unsafe path", r.logs[0])
    ...
    def test_execute_whitelist(self):
        with StatePod(self.root) as s:
            ...
            self.assertIn("1", out)
            r = s.execute([{"type": SP_OP_EXECUTE, "command": "rm -rf /"}])
            self.assertFalse(r.ok)
            self.assertIn("not whitelisted", r.logs[0])

    def test_git_diff_status(self):
        with StatePod(self.root) as s:
            s.write(".gitignore", ".statepod/\n")
            s.write("a.c", "int main(void) { return 0; }\n")
    ...
    def test_checkpoint_rollback(self):
        with StatePod(self.root) as s:
            ...
            s.rollback(ck)
            self.assertEqual(s.read("f.txt", strategy=SP_CONTEXT_FULL), "v1\n")
    ...
    def test_ast_escalation(self):
        with StatePod(self.root) as s:
            s.write("a.c", "int main(void) { return 0; }\n")
            r = s.execute([{"type": SP_OP_AST_PARSE, "path": "a.c"}])
            ...
            bad = s.execute([{"type": SP_OP_AST_QUERY, "path": "a.c", "pattern": "(("}])
    ...
    def test_context_targeted(self):
        with StatePod(self.root) as s:
            ...
            r = s.execute(
                [{"type": SP_OP_READ, "path": "t.txt"}, {"type": SP_OP_READ, "path": "other.txt"}],
                strategy=SP_CONTEXT_TARGETED, target_paths=["t.txt"],
            )
    ...
    def test_loop_stability(self):
        ...
        with StatePod(self.root) as s:
            ...
            for i in range(10):
                ...
                s.read("loop.txt", strategy=SP_CONTEXT_DELTA)
                s.grep("iteration")
