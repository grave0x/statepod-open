...
Forces SS_GREP_BACKEND=rg so the subprocess path is exercised regardless
...
os.environ["SS_GREP_BACKEND"] = "rg"  # must be set before the first kernel call
...
def make_repo(contents=None):
    d = tempfile.mkdtemp(prefix="swarmstate-rg-")
...
class RgBackendTests(unittest.TestCase):
    ...
    def test_auto_mode_large_repo_uses_rg(self):
        os.environ["SS_GREP_BACKEND"] = "auto"
        try:
            d = tempfile.mkdtemp(prefix="swarmstate-rg-big-")
            for i in range(300):
