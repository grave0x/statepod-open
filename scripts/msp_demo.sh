...
cd "$ROOT"
WEB="${SP_DEMO_WEB:-18080}"
MESH="${SP_DEMO_MESH:-17700}"

cat <<EOF
=== StatePod MSP demo (scripted) ===
Wireframes: docs/ux/00-msp-flow.uxd (+ role screens)
Security:   docs/phase4-security-gate.md (Strix still open)

1. JOIN
   Terminal A:  ./win/build/statepod-node-linux --name hub --port $MESH --web $WEB --root /tmp/sp_demo_hub
   Terminal B:  ./win/build/statepod-node-linux --name field --peer 127.0.0.1:$MESH --web $((WEB+1)) --root /tmp/sp_demo_field
...
2. DIAGNOSE
   ...
   Watch alerts: SP_WATCH_INTERVAL_MS=5000 on the node
   Tasks/STRAT:  bin/statepod stats   (from a repo with registry history)
...
3. REMEDIATE (copy-token gov on the node)
   ...
   Harness (same secret): bin/statepod gov verify --token "\$TOK" --secret demo-secret
   # D2: stop peer B → watch forces reconnect when peer returns
   # POST /api/gov/decide JSON body supported (GET query still works)

Ctrl+C nodes when done. Sandbox roots under /tmp/sp_demo_*.
EOF
mkdir -p /tmp/sp_demo_hub /tmp/sp_demo_field
