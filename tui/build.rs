// build.rs — wireframes.json guard + libstatepod link path.
...
fn main() {
    ...
    // Embed rpath so the binary finds libstatepod.so at runtime without
    ...
    println!("cargo:rerun-if-changed=../libstatepod.so");
    println!("cargo:rerun-if-changed={}/libstatepod.so", local_lib.display());
}
