...
| Widgets: status, tasks, alerts, roster, gov, QR | **SHIPPED** | `status` (kernel/hash/ops/disk/peers), `tasks` (local registry partition), `alerts`, `roster` (capability announcements), `gov` (denial counters, display-only), `qr` |
...
| Governance approve/deny from the UI | **NOT YET** | `gov` widget is display-only (denial counters); real approval flows live in the Python harness/`sw` CLI — cross-language approval is roadmap |
