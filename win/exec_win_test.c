...
static SS_Result* run_exec(RepoState* st, const char* cmd)
{
    SS_Operation op;
    SS_Plan plan;
    memset(&op, 0, sizeof(op));
    op.type = SS_OP_EXECUTE;
    op.command = cmd;
    memset(&plan, 0, sizeof(plan));
    plan.ops = &op;
    plan.op_count = 1;
    return ss_execute(st, &plan);
}
...
int main(int argc, char** argv)
{
    ...
    char detail[512];
    SS_Result* r;
    RepoState* st = ss_state_new(root);
    ...
    ss_state_set_allow_service_mgmt(st, 1);
    r = run_exec(st, "Restart-Service Spooler");
    ...
    ss_state_set_allow_service_mgmt(st, 0);   /* back to default */
    ss_state_free(st);
    printf(failures ? "\n%d FAILURES\n" : "\nALL PASS\n", failures);
    return failures ? 1 : 0;
}
